...
def forward_wrap_with_option_len(self, input_ids=None, labels=None, option_len=None, num_options=None, return_dict=None,
    ...
    if option_len is not None:  # MODIFIED corrected None issue
...
def encode_prompt(task, template, train_samples, eval_sample, tokenizer, max_length, sfc=False, icl_sfc=False,
    ...
    if hasattr(tokenizer, 'add_bos_token') and tokenizer.add_bos_token:  # temporary fix for llama3
...
class NondiffCollator(DataCollatorMixin):
    ...
    def torch_call(self, features):
        ...
        batch = self.tokenizer.pad(no_labels_features, padding=self.padding, max_length=self.max_length,
        ...
        if padding_side == "right":
            batch[label_name] = [to_list(label) + [self.label_pad_token_id] * (sequence_length - len(label)) for label
                                 in labels]
        else:
            batch[label_name] = [[self.label_pad_token_id] * (sequence_length - len(label)) + to_list(label) for label
                                 in labels]
...
def write_predictions_to_file(final_preds, output, mode="w"):
    with open(output, mode) as f:
        for pred in final_preds:
            f.write(json.dumps(pred, cls=EnhancedJSONEncoder) + "\n")


def write_metrics_to_file(metrics, output, mode="w"):
    json.dump(metrics, open(output, mode), cls=EnhancedJSONEncoder, indent=4)
